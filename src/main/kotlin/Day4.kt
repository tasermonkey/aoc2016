...
internal data class Room(val encryptedName: String, val name: String, val sectorId: Int, val checkSum: String)

internal fun parseRoom(str: String): Room {
...
internal fun getMostCommon(room: Room): Set<Char> = room.encryptedName.replace("-", "").toCharArray().groupBy { it }.entries
...
internal fun calcCheckSum(room: Room) = getMostCommon(room).joinToString("")
internal fun isValid(room: Room) = room.checkSum == calcCheckSum(room)
internal fun decryptName(name: String, rotateAmount: Int): String {
    val padRng = ('a'..'z')
    val pad = padRng.toList()
    return name.map { it -> if (padRng.contains(it)) pad[(pad.indexOf(it) + rotateAmount) % pad.size] else ' ' }.joinToString("")
}

internal fun decryptRoom(room: Room): Room = Room(room.encryptedName, decryptName(room.encryptedName, room.sectorId), room.sectorId, room.checkSum)
